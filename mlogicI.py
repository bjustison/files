...
'SQ':'Squall','PO':'Dust Whirls','DS':'Duststorm','SS':'Sandstorm','FC':'Funnel Cloud','BL':'Blowing'}
...
#NSC stands for No Significant Clouds. This is equiv to SKC and NCD. Also interpretted as empty Cloud-List
...
def __sanitize(wxData):
	...
	for i in reversed(range(len(wxData))):
		print(wxData[i])
		if wxData[i] in ['AUTO' , 'COR' , 'NSC' , 'SKC' , 'NCD' , '$']:
			print('Will pop ' + wxData[i])
			wxData.pop(i)
		if len(wxData[i]) > 3 and wxData[i][0] == 'R' and wxData[i][3] == '/' and wxData[i][1:3].isdigit():
			print('Found runway visibility')
...
def __getAltimeterUS(wxData):
	if wxData and (wxData[len(wxData)-1][0] == 'A'):
		altimeter = wxData[len(wxData)-1][1:]
		wxData.pop()
	elif wxData and len(wxData[len(wxData)-1]) == 4 and wxData[len(wxData)-1].isdigit():
...
def __getWindInfo(wxData):
	...
	variable = []
	if wxData and ((wxData[0][len(wxData[0])-2:] == 'KT') or (wxData[0][len(wxData[0])-3:] == 'KTS') or (len(wxData[0]) == 5 and wxData[0][0] != 'A') or (len(wxData[0]) >= 8 and wxData[0].find('G') != -1 and wxData[0].find('/') == -1)):
		...
		wxData.pop(0)
	elif wxData and len(wxData[0]) == 1 and wxData[0] == 'M':  #The "Calm M"
		wxData.pop(0)
	elif wxData and len(wxData[0]) > 5 and wxData[0][3] == '/' and  wxData[0][:3].isdigit() and  wxData[0][:3].isdigit():
		direction = wxData[0][:3]
		if wxData[0].find('G') != -1:
		...
		wxData.pop(0)
	#Loose "KT"
	if wxData and len(wxData[0]) == 2 and wxData[0] == 'KT': wxData.pop(0)
	#Separated Gust
...
def __getVisibilityUS(wxData):
	...
	if wxData and (wxData[0].find('SM') != -1):   #10SM
		visibility = wxData[0][:wxData[0].find('SM')]
		wxData.pop(0)
...
def __getVisibilityInternational(wxData):
	visibility = ''
	if wxData and len(wxData[0]) == 4 and wxData[0].isdigit():
...
def getCeiling(clouds):
	for cloud in clouds:
		if (cloud[1].find('/') == -1) and (cloud[0] in ['OVC','BKN','VV']):
			return cloud
	return None
